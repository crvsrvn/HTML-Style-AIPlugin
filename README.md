...
| 大纲 | 右侧整高侧栏，无底色，从标题自动生成，可筛选、随滚动高亮；默认折叠到第一层，点第一层标题展开其子标题（同时只展开一个），图标按钮切换展开全部；窄于 900px 时收成右上角按钮 + 右侧抽屉 |
| 标题与强调 | 标题带序号（与标题同色）、不加图标；加粗与高亮（`<mark>`）二选一 |
| 色彩与标记 | 章节色（红橙黄绿青蓝紫 7 色循环）；语义提示块（说明 / 建议 / 注意 / 风险 / 要点 / 备注）；状态标签、图例记号、ID 自动着色；内置 33 个线性图标 |
