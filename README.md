...
| 大纲 | 右侧整高侧栏，无底色，从标题自动生成，可筛选、随滚动高亮；窄于 900px 时收成右上角按钮 + 右侧抽屉 |
| 标题与强调 | 标题不加序号和图标；加粗与高亮（`<mark>`）二选一 |
| 色彩与标记 | 章节色（8 色循环）；语义提示块（说明 / 建议 / 注意 / 风险 / 要点 / 备注）；状态标签、图例记号、ID 自动着色；内置 31 个线性图标 |
