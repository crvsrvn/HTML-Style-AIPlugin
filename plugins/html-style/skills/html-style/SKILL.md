---
name: html-style
description: 创建、修改、重排或发布任何 HTML 成品时必须使用——包括 HTML 文档、报告、方案、说明页、仪表盘、Claude Artifacts、把 Markdown 转成 HTML、更新已有 Artifact 链接。统一样式：Comic Sans MS 16px、右侧大纲、底色 #FCFCFB / #151515、988px 居中版心；用章节色、图标、语义提示块、状态标签、图例记号辅助阅读；SVG 等图表可左右键拖动、滚轮定点缩放、双击全屏，并尽量用原生 HTML/CSS 提升可读性；成品托管到 AI 云端（如 Claude Artifacts），不交付本地 .html 文件。
---
...
| 大纲 | 放在右侧：固定的整高侧栏，不加底色；由模板脚本从标题生成，可筛选，随滚动用章节色文字和左侧细条标出当前位置；视口窄于 900px 时收进右上角「大纲」按钮，从右侧滑出 |
| 版心 | 固定宽度 988px，在大纲左侧的区域内水平居中；空间不足时收窄，两侧至少留 16px |
| 标题 | 章节标题带序号：`h2` 写 `1.`，`h3` 写 `1.1`，`h4` 写 `1.1.1`，统一用 `<span class="hs-num">1.</span>` 包裹；序号与标题文字同色，不着章节色；标题不加图标 |
...
| 类别 | 取值 | 含义 |
|------|------|------|
| 章节色 | `--hs-c1` … `--hs-c8`：蓝、青、紫、玫红、湖蓝、靛、绿、琥珀 | 属于哪一章。脚本按 `h2` 顺序自动分配；需要指定时在 `h2` 上写 `data-hue="1"`…`"8"` |
| 语义色 | `data-kind`：`info` 说明（蓝）、`tip` 建议（绿）、`ok` 通过（绿）、`warn` 注意（琥珀）、`danger` 风险（红）、`key` 要点（紫）、`muted` 备注（灰） | 内容的性质或状态 |
| ID 色 | 脚本自动分配 | 同一前缀的编号（`RT-01`、`RT-02`）同色，不同前缀异色 |

章节色自动作用于：`h2` 色条、`h3` 竖条、表头、列表符号、大纲章节名与当前位置。
...
可用图标：`info` `tip` `ok` `warn` `danger` `flag` `note` `question` `book` `target` `compare` `database` `flow` `code` `image` `list` `calendar` `flask` `layers` `branch` `clock` `link` `zap` `box` `shield` `migrate` `search` `arrow-up` `expand` `shrink` `reset`。
...
- [ ] 视口 ≥900px 时大纲是右侧整高侧栏，没有底色；更窄时有「大纲」按钮和右侧抽屉。
...
- [ ] 章节色、提示块、状态标签、图例记号、ID 标记都已用上，且每种颜色都有明确含义。
