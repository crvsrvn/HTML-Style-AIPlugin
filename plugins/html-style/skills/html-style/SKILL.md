...
| 大纲 | 放在右侧：固定的整高侧栏，不加底色；由模板脚本从标题生成，可筛选，随滚动用章节色文字和左侧细条标出当前位置；视口窄于 900px 时收进右上角「大纲」按钮，从右侧滑出。<br>折叠 / 展开：<br>• 默认折叠，只显示第一层标题<br>• 点第一层标题跳转并展开它的下一层子标题；点另一个第一层标题时，之前展开的收起<br>• 标题栏右侧的纯图标按钮切换「展开全部 / 折叠」，展开时显示全部标题<br>• 筛选时显示全部匹配项；当前位置被折叠时，标记落在所属的第一层标题上 |
...
| 类别 | 取值 | 含义 |
|------|------|------|
| 章节色 | `--hs-c1` … `--hs-c7`：红、橙、黄、绿、青、蓝、紫 | 属于哪一章。脚本按 `h2` 顺序依次取色，第 8 章起从红色重新循环；需要指定时在 `h2` 上写 `data-hue="1"`…`"7"` |
...
- [ ] 视口 ≥900px 时大纲是右侧整高侧栏，没有底色，默认折叠到第一层，折叠 / 展开按钮只有图标；更窄时有「大纲」按钮和右侧抽屉。
