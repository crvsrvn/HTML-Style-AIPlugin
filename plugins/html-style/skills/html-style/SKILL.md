...
| 色彩与标记 | 必须用多种颜色、图标、标记辅助阅读（见[色彩、图标与标记](#色彩图标与标记)）：每个 `h2` 配图标；提示性段落用语义提示块；状态词用状态标签；图例符号用记号。颜色必须编码含义，不做纯装饰 |
...
章节色自动作用于：`h2` 图标块和色条、`h3` 竖条、标题编号、表头、列表符号、粗体高亮、大纲条目。

标记写法：

| 标记 | 写法 | 用在哪里 |
|------|------|----------|
| 章节图标 | `<h2 data-icon="database">` | 每个 `h2` 必须配一个与内容相符的图标，大纲同步显示 |
...
5. 按[色彩、图标与标记](#色彩图标与标记)和[原生可读性增强](#原生可读性增强)组织内容：给每个 `h2` 选图标，把提示性段落写成提示块，把状态词写成状态标签。
...
4. 按"新建页面"第 2–6 步套上模板：
   ...
   - 补齐标记：给每个 `h2` 配 `data-icon`；逐个判断旧引用块的性质，改成对应 `data-kind` 的提示块并加标题；表格里整格是状态词的改成状态标签；文档自定义的图例符号改成图例记号。
...
- [ ] 每个 `h2` 都有图标，章节色、提示块、状态标签、图例记号、ID 标记都已用上，且每种颜色都有明确含义。
