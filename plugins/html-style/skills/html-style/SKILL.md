...
| 标题 | 不加序号前缀，不加图标。唯一例外：正文按章节号交叉引用（如"见 4.3"）时保留编号，写成 `<span class="hs-num">4.3</span>` |
...
5. 按[色彩、图标与标记](#色彩图标与标记)和[原生可读性增强](#原生可读性增强)组织内容：标题不加序号和图标，把提示性段落写成提示块，把状态词写成状态标签，强调时在加粗与高亮之间二选一。
...
4. 按"新建页面"第 2–6 步套上模板：
   - 去掉标题里的序号和图标；正文按章节号交叉引用时保留编号，写成 `hs-num`。
...
- [ ] 标题没有序号和图标（按章节号交叉引用的文档除外）；加粗与高亮没有叠用。
